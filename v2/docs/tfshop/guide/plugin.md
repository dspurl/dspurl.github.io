...
```
#### `coupon/tfshop.json`
- 这里的tfshop.json是 `coupon`插件目录下的配置文件
 ```markdown
{
    "name": "优惠券",  //插件名称
...
- 使用别人的开发模式代码，请不要使用第二步操作，可进行一次插件安装，然后将开发模式下的`tfshop.json`替换，即会自动切换到开发模式
## 插件开发
- 首先请确保你已经了解插件的结构
- 你想开发的插件是无耦合的，且可扩展的
...
```
### 第七步：测试
- 下载最新版本的tfshop，根据插件安装流程，安装自己的插件，看最终是否可以正常安装、使用和卸载
